...
class RegistroArmaWorkflow implements iAction
{
  ...
  public function apply(Array $arr) {
    $registro_arma_id = $arr["object_id"];
    $action = $arr["action"];
    $user_email = $arr["user_email"];
    $registro_arma = RegistroArma::find($registro_arma_id);

    # set enabled transitions
    $result = new \stdClass;
    try {
      $arr = $this->tools->get_workflow_transitions($registro_arma, $action, $user_email);

      # set initial state if workflow_state is null
      if (is_null($registro_arma->workflow_state)) { $registro_arma->workflow_state = $this->state;}

      # apply workflow transition
      try {
          $res = $this->tools->workflow_apply($registro_arma, $action);
      } catch (\Exception $e) {
          return result;
      }
      ...
      $registro_arma->save();
      $result->success = true;
      $result->message = $registro_arma;
      return $result;
    } catch (\Exception $e) {
      unset($registro_arma["enabled_transitions"]);
      unset($registro_arma["user_email"]);
  ...
  public function user_actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->allowed_actions($registro_arma, $this->workflow_name, $user_email);
    $result->message = $arr;
    return $result;
  }
  ...
  public function actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->get_actions($registro_arma);
    $result->message = $arr;
    return $result;
  }
  ...
  public function dependencia(RegistroArma $registro_arma) {
    $dependencia_id = NULL;
    $d_mp = RegistroArma::whereId($registro_arma->id)->with('institucion.documento.dependencia')->first();
