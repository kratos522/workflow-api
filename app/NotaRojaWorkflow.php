...
class NotaRojaWorkflow implements iAction
{
  ...
  public function __construct($remove_root=false)
  {
    ...
    $this->workflow_name = "retencion_nota_roja";
  ...
  public function apply(Array $arr) {
    $nota_roja_id = $arr["object_id"];
    $action = $arr["action"];
    $user_email = $arr["user_email"];
    $nota_roja = NotaRoja::find($nota_roja_id);

    # set enabled transitions
    $result = new \stdClass;
    try {
      $arr = $this->tools->get_workflow_transitions($nota_roja, $action, $user_email);

      # set initial state if workflow_state is null
      if (is_null($nota_roja->workflow_state)) { $nota_roja->workflow_state = $this->state;}

      # apply workflow transition
      try {
          $res = $this->tools->workflow_apply($nota_roja, $action);
      } catch (\Exception $e) {
          return result;
      }
      ...
      $nota_roja->save();
      $result->success = true;
      $result->message = $nota_roja;
      return $result;
    } catch (\Exception $e) {
      unset($nota_roja["enabled_transitions"]);
      unset($nota_roja["user_email"]);
  ...
  public function user_actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->allowed_actions($nota_roja, $this->workflow_name, $user_email);
    $result->message = $arr;
    return $result;
  }
  ...
  public function actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->get_actions($nota_roja);
    $result->message = $arr;
    return $result;
  }
  ...
  public function dependencia(NotaRoja $nota_roja) {
    $dependencia_id = NULL;
    $d_mp = NotaRoja::whereId($nota_roja->id)->with('institucion.documento.dependencia')->first();
