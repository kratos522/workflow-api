...
class IntervencionComunicacionWorkflow implements iAction
{
  ...
  public function __construct($remove_root=false)
  {
    ...
    $this->workflow_name = "intervencion_comunicacion";
    ...
    public function apply(Array $arr) {
      $intervencion_comunicacion_id = $arr["object_id"];
      $action = $arr["action"];
      $user_email = $arr["user_email"];
      $intervencion_comunicacion = IntervencionComunicacion::find($intervencion_comunicacion_id);

      # set enabled transitions
      $result = new \stdClass;
      try {
        $arr = $this->tools->get_workflow_transitions($intervencion_comunicacion, $action, $user_email);

        # set initial state if workflow_state is null
        if (is_null($intervencion_comunicacion->workflow_state)) { $intervencion_comunicacion->workflow_state = $this->state;}

        # apply workflow transition
        try {
            $res = $this->tools->workflow_apply($intervencion_comunicacion, $action);
        } catch (\Exception $e) {
            return result;
        }

        $intervencion_comunicacion->save();
        $result->success = true;
        $result->message = $intervencion_comunicacion;
        return $result;
      } catch (\Exception $e) {
        unset($intervencion_comunicacion["enabled_transitions"]);
        unset($intervencion_comunicacion["user_email"]);
    ...
    public function user_actions(Array $arr) {
      ...
      $result = new \stdClass;
      $result->success = true ;
      $arr = $this->tools->allowed_actions($intervencion_comunicacion, $this->workflow_name, $user_email);
      $result->message = $arr;
      return $result;
    }
    ...
    public function actions(Array $arr) {
      ...
      $result = new \stdClass;
      $result->success = true ;
      $arr = $this->tools->get_actions($intervencion_comunicacion);
      $result->message = $arr;
      return $result;
    }
    ...
    public function dependencia(IntervencionComunicacion $intervencion_comunicacion) {
      $dependencia_id = NULL;
      $d_mp = IntervencionComunicacion::whereId($intervencion_comunicacion->id)->with('institucion.documento.dependencia')->first();
