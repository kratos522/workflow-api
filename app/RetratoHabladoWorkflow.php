...
class RetratoHabladoWorkflow implements iAction
{
  ...
  public function __construct($remove_root=false)
  {
    ...
    $this->workflow_name = "unidad_retrato_hablado";
  ...
  public function apply(Array $arr) {
    $retrato_hablado_id = $arr["object_id"];
    $action = $arr["action"];
    $user_email = $arr["user_email"];
    $retrato_hablado = RetratoHablado::find($retrato_hablado_id);

    # set enabled transitions
    $result = new \stdClass;
    try {
      $arr = $this->tools->get_workflow_transitions($retrato_hablado, $action, $user_email);

      # set initial state if workflow_state is null
      if (is_null($retrato_hablado->workflow_state)) { $retrato_hablado->workflow_state = $this->state;}

      # apply workflow transition
      try {
          $res = $this->tools->workflow_apply($retrato_hablado, $action);
      } catch (\Exception $e) {
          return result;
      }

      $retrato_hablado->save();
      $result->success = true;
      $result->message = $retrato_hablado;
      return $result;
    } catch (\Exception $e) {
      unset($retrato_hablado["enabled_transitions"]);
      unset($retrato_hablado["user_email"]);
  ...
  public function user_actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->allowed_actions($retrato_hablado, $this->workflow_name, $user_email);
    $result->message = $arr;
    return $result;
  }
  ...
  public function actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->get_actions($retrato_hablado);
    $result->message = $arr;
    return $result;
  }
  ...
  public function dependencia(RetratoHablado $retrato_hablado) {
    $dependencia_id = NULL;
    $d_mp = RetratoHablado::whereId($retrato_hablado->id)->with('institucion.documento.dependencia')->first();
