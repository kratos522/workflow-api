...
class ExtraccionInformacionTelefonoMovilWorkflow implements iAction
{
  ...
  public function apply(Array $arr) {
    $extraccion_informacion_telefono_movil_id = $arr["object_id"];
    $action = $arr["action"];
    $user_email = $arr["user_email"];
    $extraccion_informacion_telefono_movil = ExtraccionInformacionTelefonoMovil::find($extraccion_informacion_telefono_movil_id);

    # set enabled transitions
    $result = new \stdClass;
    try {
      $arr = $this->tools->get_workflow_transitions($extraccion_informacion_telefono_movil, $action, $user_email);

      # set initial state if workflow_state is null
      if (is_null($extraccion_informacion_telefono_movil->workflow_state)) { $extraccion_informacion_telefono_movil->workflow_state = $this->state;}

      # apply workflow transition
      try {
          $res = $this->tools->workflow_apply($extraccion_informacion_telefono_movil, $action);
      } catch (\Exception $e) {
          return result;
      }

      $extraccion_informacion_telefono_movil->save();
      $result->success = true;
      $result->message = $extraccion_informacion_telefono_movil;
      return $result;
    } catch (\Exception $e) {
      unset($extraccion_informacion_telefono_movil["enabled_transitions"]);
      unset($extraccion_informacion_telefono_movil["user_email"]);
  ...
  public function user_actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->allowed_actions($extraccion_informacion_telefono_movil, $this->workflow_name, $user_email);
    $result->message = $arr;
    return $result;
  }
  ...
  public function actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->get_actions($extraccion_informacion_telefono_movil);
    $result->message = $arr;
    return $result;
  }
  ...
  public function dependencia(ExtraccionInformacionTelefonoMovil $extraccion_informacion_telefono_movil) {
    $dependencia_id = NULL;
    $d_mp = ExtraccionInformacionTelefonoMovil::whereId($extraccion_informacion_telefono_movil->id)->with('institucion.documento.dependencia')->first();
