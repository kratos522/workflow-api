...
class CotejamientoDactilarWorkflow implements iAction
{
  ...
  public function __construct($remove_root=false)
  {
    ...
    $this->workflow_name = "cotejamiento_dactilar";
    ...
    public function apply(Array $arr) {
      $cotejamiento_dactilar_id = $arr["object_id"];
      $action = $arr["action"];
      $user_email = $arr["user_email"];
      $cotejamiento_dactilar = CotejamientoDactilar::find($cotejamiento_dactilar_id);

      # set enabled transitions
      $result = new \stdClass;
      try {
        $arr = $this->tools->get_workflow_transitions($cotejamiento_dactilar, $action, $user_email);

        # set initial state if workflow_state is null
        if (is_null($cotejamiento_dactilar->workflow_state)) { $cotejamiento_dactilar->workflow_state = $this->state;}

        # apply workflow transition
        try {
            $res = $this->tools->workflow_apply($cotejamiento_dactilar, $action);
        } catch (\Exception $e) {
            return result;
        }

        $cotejamiento_dactilar->save();
        $result->success = true;
        $result->message = $cotejamiento_dactilar;
        return $result;
      } catch (\Exception $e) {
        unset($cotejamiento_dactilar["enabled_transitions"]);
        unset($cotejamiento_dactilar["user_email"]);
    ...
    public function user_actions(Array $arr) {
      ...
      $result = new \stdClass;
      $result->success = true ;
      $arr = $this->tools->allowed_actions($cotejamiento_dactilar, $this->workflow_name, $user_email);
      $result->message = $arr;
      return $result;
    }
    ...
    public function actions(Array $arr) {
      ...
      $result = new \stdClass;
      $result->success = true ;
      $arr = $this->tools->get_actions($cotejamiento_dactilar);
      $result->message = $arr;
      return $result;
    }
    ...
    public function dependencia(CotejamientoDactilar $cotejamiento_dactilar) {
      $dependencia_id = NULL;
      $d_mp = CotejamientoDactilar::whereId($cotejamiento_dactilar->id)->with('institucion.documento.dependencia')->first();
