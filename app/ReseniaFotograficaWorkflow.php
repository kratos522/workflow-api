...
class ReseniaFotograficaWorkflow implements iAction
{
  ...
  public function __construct($remove_root=false)
  {
    ...
    $this->workflow_name = "realizar_reseña_fotografica";
  ...
  public function apply(Array $arr) {
    $reseña_fotografica_id = $arr["object_id"];
    $action = $arr["action"];
    $user_email = $arr["user_email"];
    $reseña_fotografica = ReseniaFotografica::find($reseña_fotografica_id);

    # set enabled transitions
    $result = new \stdClass;
    try {
      $arr = $this->tools->get_workflow_transitions($reseña_fotografica, $action, $user_email);

      # set initial state if workflow_state is null
      if (is_null($reseña_fotografica->workflow_state)) { $reseña_fotografica->workflow_state = $this->state;}

      # apply workflow transition
      try {
          $res = $this->tools->workflow_apply($reseña_fotografica, $action);
      } catch (\Exception $e) {
          return result;
      }

      $reseña_fotografica->save();
      $result->success = true;
      $result->message = $reseña_fotografica;
      return $result;
    } catch (\Exception $e) {
      unset($reseña_fotografica["enabled_transitions"]);
      unset($reseña_fotografica["user_email"]);
  ...
  public function user_actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->allowed_actions($reseña_fotografica, $this->workflow_name, $user_email);
    $result->message = $arr;
    return $result;
  }
  ...
  public function actions(Array $arr) {
    ...
    $result = new \stdClass;
    $result->success = true ;
    $arr = $this->tools->get_actions($reseña_fotografica);
    $result->message = $arr;
    return $result;
  }
  ...
  public function dependencia(ReseniaFotografica $reseña_fotografica) {
    $dependencia_id = NULL;
    $d_mp = ReseniaFotografica::whereId($reseña_fotografica->id)->with('institucion.documento.dependencia')->first();
